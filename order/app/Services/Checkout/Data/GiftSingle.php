...
//            }
            foreach ($applied_rule_ids as $_rule_id){
                ...
                if(($_rule_id['type'] == 'gift' or $sub_type == 'code_gift') and $_rule_id['is_special_gift'] != '1'){
                    $fit_rule_id_arr[] = $_rule_id['rule_id'];
                }
            }
...
//        }
    ...
    private function groupMainGift(){
        ...
        //去除items中相同的rule下的gift只保留name
        $this->uniqueRules($cart_items);
loopAgain:
...
//        $this->data['debug_group'][] = $group;
     ...
     * @param $cart_items
     */
    private function uniqueRules(&$cart_items){
        $same_rules = [];
        foreach($cart_items as &$cart_item){
            foreach($cart_item['applied_rule_ids'] as $k=>$rule){
                if(!in_array($rule['rule_id'],$same_rules)){
                    $same_rules[] = $rule['rule_id'];
                }else{
                    $cart_item['applied_rule_ids'][$k]['remove_gifts'] = 1;
                }
            }
        }
    }
...
//                $gift_items = $this->getGiftSkusByRuleId($rule_id,$gift_items);
                $remove_gifts = array_get($applied_rule,'remove_gifts');
                if($remove_gifts!=1){
                    $same_rule[] = $rule_id;
                }
                $same_rule_name[] = $rule_id;
            }
        }
...
//        }
        ...
        $gift_rule_name_arr = [];
        $same_rule_name = array_unique($same_rule_name);
        foreach($same_rule_name as $rule_id){
