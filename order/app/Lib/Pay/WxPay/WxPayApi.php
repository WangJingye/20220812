...
use App\Lib\Pay\WxPay\WxPayException;
use App\Lib\Pay\WxPay\WxPayConfigInterface;

use App\Lib\Pay\WxPay\WxData\WxPayResults;
use App\Lib\Pay\WxPay\WxData\WxPayNotifyResults;
use App\Lib\Pay\WxPay\WxData\WxPayReport;
...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($config, $inputObj, $timeOut = 10)
	{
		Log::info('支付信息xml',[$inputObj]);
		...
		//Log::info('notify_url'. json_encode($config->GetNotifyUrl()));
		...
		$inputObj->SetSpbill_create_ip(self::getRealIp());//终端ip
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		//签名

		$inputObj->SetSign($config);
		Log::info('支付信息xml_json',[$inputObj]);
		$xml = $inputObj->ToXml();

		Log::info('支付信息xml'.json_encode($xml));
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($config, $xml, $url, false, $timeOut);

		$result = WxPayResults::Init($config, $response);
		Log::info('支付信息'.json_encode($result));
	 ...
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	 * @param WxPayConfigInterface $config  配置对象
	 * @param \App\Lib\Pay\WxPay\WxData\WxPayRefund $inputObj
	 * @param int $timeOut
	 * @throws WxPayException
	 * @return 成功时返回，其他抛异常
	 */
	...
	private static function postXmlCurl($config, $xml, $url, $useCert = false, $second = 30)
	{
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, '/opt/wechat_pem/el_cert.pem');
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, '/opt/wechat_pem/el_key.pem');
		}
		//post提交方式
