...
use App\Lib\Http;
use App\Lib\Pay\WxPay\WxData\WxPayJsApiPay;
...
class JsApiPay
{
	 ...
	 * @return 用户的openid
	 */
	public function GetOpenid($from = '')
	{
		//通过code获得openid
		if (!isset($_GET['code'])){
			//触发微信返回code码
			$baseUrl = urlencode(env('NOTIFY_DOMAIN').'/order/api/pay/wechatNotify?url='.$from);
			//$baseUrl = urlencode(env('NOTIFY_DOMAIN').'/order/api/pay/wechatNotify');
