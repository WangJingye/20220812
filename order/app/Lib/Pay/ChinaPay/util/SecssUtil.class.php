...
class SecssUtil
{
	...
	private function number_to_binary($number, $blocksize)
	{
		...
		return str_pad($result, $blocksize, "\0", STR_PAD_LEFT);
	}
	...
	public function encryptData($data)
	{
		try {
			...
			foreach (str_split($tmpData, $split_length) as $chunk) {
				if (openssl_public_encrypt(str_pad($chunk, $pad_length, "\0", STR_PAD_LEFT), $encryptData, $pk, OPENSSL_NO_PADDING)) {
					$crypted .= $encryptData;
				} else {
					$is_error = 1;
				}
