...
class OrderCurlLog
{
    ...
    protected function curl_info(array $request_info){
        ...
        if(env("APP_ENV") !== "local"){
            if(!file_exists("/var/www/html/css/Log")){
                mkdir("/var/www/html/css/Log", 0777);
            };
            file_put_contents('/var/www/html/css/Log/Curl_Success' . $this->log_frag .  ".Log",
    ...
    protected function curl_warning(array $request_info)
    {
        ...
        if(env("APP_ENV") !== "local"){
            ...
            file_put_contents('/var/www/html/css/Log/Curl_Fail' . $this->log_frag .  ".Log",
                json_encode($request_info)."\r\n", FILE_APPEND);
        }else {
            if (!file_exists("Log")) {
                mkdir("Log");
    ...
    protected function curl_error(array $response_info)
    {
        //非开发模式下，日志存储到 /var/www/html/css/Log 这个目录下
        if(env("APP_ENV") !== "local"){
            if(!file_exists("/var/www/html/css/Error")){
                mkdir("/var/www/html/css/Error", 0777);
            };
            file_put_contents('/var/www/html/css/Error/Curl_Error' . $this->log_frag .  ".Log",
                json_encode($response_info)."\r\n", FILE_APPEND);
        }else {
            if (!file_exists("Error")) {
                mkdir("Error");
            };
            file_put_contents('Error/Curl_Error' . $this->log_frag . ".Log",
                json_encode($response_info) . "\r\n", FILE_APPEND);
        }
    }
