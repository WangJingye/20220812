...
class PayController extends ApiController
{
    ...
    public function pay(Request $request)
    {
        ...
        $result = app('ApiRequestInner', ['module' => 'oms'])->request('orders/details', 'GET', ['order_sn' => $order_sn]);
        if ($result['code'] == 0) return false;
        $orderInfo = $result['data'][0];
        $payAmount = $orderInfo['total_amount'];
        ...
        if ($type == 11 || $type == 10 || $type == 'MultiPay'|| $type == 'GoldPay') {
            $result = app('ApiRequestInner', ['module' => 'member'])
                ->request('user/getBalance', 'GET', ['user_id' => $this->getUid()]);
            if ($result['code'] == 0) return false;
            $balance = $result['data']['balance'];

            if ($type == 10 || $type == 'GoldPay') {//余额支付
                if ($balance < $payAmount) {
                    return $this->error([], '储值余额不足，无法支付');
                } else {

                    $params = [
                        'pay_order_sn' => $orderInfo['order_sn'],
                        'order_sn' => $order_sn,
                        'type' => array_search($type, $this->getTypeDB()),
                        'tradeType' => array_search($trade_type, $this->getTradeTypeDB()),
                        'time' => date('Y-m-d H:i:s'),
                        'payment' => json_encode(['memo' => '储值卡支付', 'amount' => $payAmount]),
                        'huabei_period' => $request->get('hb_fp_num', 0),
                        'trade_no' => $order_sn,
                        'payTime' => date('Y-m-d H:i:s'),
                        'pay_amount' => 0,
                        'gold_amount' => $payAmount,
                    ];
                    Log::info('pay 发起支付参数', [$params]);
                    $api = app('ApiRequestInner', ['module' => 'oms']);
                    $payInfo = $api->request('pay/updateAndSuccess', 'POST', $params);
                    Log::info('pay 发起支付请求oms返回结果', [$payInfo]);
                    if (isset($payInfo) && $payInfo['code'] == 0) {
                        return $this->error([], $payInfo['message']);
                    }
                    return $this->success();
                }
            } else if ($type == 11 || $type == 'MultiPay') {//组合支付
                if ($balance >= $payAmount) {
                    return $this->error([], '储值余额足够，无法使用组合支付');
                }
                if ($balance <= 0) {
                    return $this->error([], '没有储值余额，不能使用组合支付');
                }
                $payAmount -= $balance;
            }
        ...
        $params = [
            ...
            'total_amount' => $payAmount, //订单金额  分为单位
        ...
        if ($result['code'] == 1) {
            ...
            $params = [
                ...
                'gold_amount' => $orderInfo['total_amount'] - $originalPayAmount,
...
//            $refund_fee = bcdiv($refund_fee, 1000, 2);
    ...
    public function getPayType($type = '', $trade_type = '')
    {
        ...
        if ($type == 'AliPay' or $type == 'HuabeiPay' or $type == 1 or $type == 4) {
            ...
            $result['type'] = 'Wx';
            $result['trade_type'] = $trade_type;
        } elseif ($type == 'GoldPay' or$type == 10) {
            $result['type'] = 'Gold';
            $result['trade_type'] = $trade_type;
        }
        return $result;
