<template>
    <div class="block  image_on_text_swiper" tag="image_on_text_swiper" >
                ...
                <swiper  ref="jackSwiper" :options="swiperAuto"   :auto-update="true" :auto-destroy="true"
                    ...
                    <swiper-slide  v-for="(item,i) in element.nodes" :key="i">
                        <div style="position:relative">
                            <img :src="item.src"/>
                            <div class="on_text" style="position:absolute;left:0;top:0;width:100%;height:100%;">
                                <div class="flex_center;">
                                    <div>
                                        <div class="title">{{item.on_text.title}}</div>
                                        <div class="desc">{{item.on_text.desc}}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    export default {
        extends: driver,
        props:['element'],
        name: "image-on-text-swiper-show",
        data() {
            return {
                swiperOption: {
                    slidesPerView: 1,
