...
class OrderController extends Controller
{
    ...
    public function _export(Request $request)
    {
        ...
        foreach ($list as $order) {
            ...
            //组合支付
            if ($order['payment_type'] == 11) {
                $sources = [2 => '微信', 10 => '储值卡'];
            } else {
                $sources = [$order['payment_type'] => $pay_list[$order['payment_type']]];
            }
...
//                    $goods['applied_rule_ids'] = str_replace(",", "-", $goods['applied_rule_ids']);
                    ...
                    $item = [
                        ...
                        $source,
                        $paymentType == 10 ? $order['gold_amount'] : $order['pay_amount'],
        ...
        $result = [
            ...
            'columns' => [
                '订单创建日期', '会员编号', '支付时间', '发货时间', '订单号', '订单类型', '商品类型', '是否赠品', '是否小样',
                '订单状态', '状态信息', '支付流水号', '顾客类型', '快递单号', '省份', '城市', '收件人', 'SPU', 'SKU', 'UPC',
                '产品名称', '系列', '子品类', '主品类', '零售价', '销售价', '折扣率', '优惠金额', '折后价格', '数量', '销售总额',
                '货值', '支付方式', '支付来源', '支付金额', '平台类型', '促销信息', '促销规则id', '优惠码', '优惠码信息', '优惠券ID', '优惠券', '备注',
                '快递地址', '活动', '活动入口', '分享人', '手机号', '异常标识', '收货时间', '评论内容', '产品评分', '客服评分', '物流评分', '评论时间',
    ...
    public function invoice(Request $request)
    {
        ...
        return $this->curl('oms/order/invoice', $all);
    }
