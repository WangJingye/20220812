                ...
                <div class="layui-form-item"-->
                    ...
                    <div class="layui-inline">
                        <span class="layui-btn sub " id="search">搜索</span>
                         <span class="layui-btn sub layui-btn-warm" id="add">创建活动</span>
                    </div>
                </div>
            </form>
...
@section('layui_script')
    var dataTableIns = table.render({
        ...
        ,id: 'table_list'
        ,height: 500
        ,url: "{{ route('backend.trial.dataList') }}" //数据接口
        ,page: true //开启分页
        ,method:'post'
        ,cols: [[ //表头
            ...
            ,{field: 'name', title: '名称'}
            ,{field: 'money', title: '邮费'}
            ,{field: 'add_sku', title: 'sku 信息'}
...
//                 fixed: false,
                maxmin: false,
                content: "{{ route('backend.trial.update') }}?id="+data.id,
                end: function(){
...
//                 fixed: false,
                maxmin: false,
                content: "{{ route('backend.trial.view') }}?id="+data.id,
                end: function(){
//                     active['reload'].call(this);
                }
            });
//             layer.full(index);
            ...
            layer.confirm('确认激活么', function(index){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('backend.trial.active') }}",
                    data: {id:data.id},
            ...
            layer.confirm('确认禁用么', function(index){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('backend.trial.unactive') }}",
                    data: {id:data.id},
                    success: function(res){
                        if( res.code == 1 ){
                            cssReloadTable();
...
//          var cr=dataTable.config.page.curr ?dataTable.config.page.curr:1;
    ...
    $('#add').on('click', function () {
        var index = layer.open({
            ...
            offset: 't',
            maxmin: false,
            content: "{{ route('backend.trial.add') }}?type="+$(this).attr('id'),
            end: function(){
                var type = 'reload';
//                 active[type] ? active[type].call(this) : '';
                cssReloadTable();
