    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                <a class="layui-btn layui-btn-normal" href="{{ route('backend.permission.create') }}">添 加</a>
                <button class="layui-btn" id="returnParent" pid="0">返回上级</button>
            </div>
        </div>
...
@section('layui_script')
    ...
    table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'del'){
            layer.confirm('确认删除吗？', function(index){
                $.post("{{ route('backend.permission.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                    if (result.code==0){
            ...
            location.href = '{{url('admin')}}/permission/'+data.id+'/edit';
    ...
    $("#listDelete").click(function () {
        ...
        if (ids.length>0){
            layer.confirm('确认删除吗？', function(index){
                $.post("{{ route('backend.permission.destroy') }}",{_method:'delete',ids:ids},function (result) {
                    if (result.code==0){
                        dataTable.reload()
                    }
                    layer.close(index);
