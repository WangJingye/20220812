...
@section('content')
    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <h2>用户 【{{$user->name}}】分配权限 此权限于角色拥有的权限不冲突</h2>
        </div>
        <div class="layui-card-body">
            <form action="{{route('backend.user.assignPermission',['user'=>$user])}}" method="post" class="layui-form">
                {{csrf_field()}}
                {{method_field('put')}}
                ...
                <div class="layui-form-item">
                    ...
                    <a href="{{route('backend.user')}}"  class="layui-btn" >返 回</a>
                </div>
