                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input" type="text" name="title" id="product_name"
                               lay-verify="required" data-raw="{{$detail->title??old('title')}}"
                               value="{{$detail->title??old('title')}}" placeholder="如：product_ad" disabled=true>
                    </div>
                </div>
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input" type="text" name="start_time"
                               data-raw="{{$detail->start_time??old('start_time')}}"
                               value="{{$detail->start_time??old('start_time')}}" placeholder="">
                    </div>
                </div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input" type="text" name="end_time"
                               value="{{$detail->end_time??old('end_time')}}" >
                    </div>
                </div>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input" type="text" name="remark"
                               value="{{$detail->remark??old('remark')}}" >
                    </div>
                </div>
...
<script>
    ...
    layui.use(['form','upload'], function () {
        ...
        form.on('submit(confirm)', function (data) {
            $.post("{{ route('backend.ad.location.update') }}", data.field, function (res) {
                if (res.code === 1) {
                    layer.msg('操作成功', {
                        icon: 1,
                        shade: 0.3,
