        ...
        <div class="layui-card">
            <div class="layui-card-body">
                ...
                <fieldset class="layui-field-title" style="margin-top: 30px;">
                    <legend>前台页面地址:</legend>
                    <legend>首页 : /pages/index/index</legend>
                    <legend>活动页id为页面参数 : /pages/activity/activity?id=</legend>
                    <legend>商品分类 : /pages/category/category</legend>
                    <legend>个人中心 : /pages/ucenter/ucenter</legend>
                    <legend>商品列表id为分类id : /pages/pdt-list/pdt-list?id=</legend>
                    <legend>商品详情id为商品spuid : /pages/pdt-detail/pdt-detail?id=</legend>
                    <legend>入会页面 : /pages/member-clause/member-clause</legend>
                </fieldset>

                <div class="layui-container " >
                    <span class="layui-inline layui-btn" id="J_add_ad">新增广告</span>
                    <form ref="form" class="layui-form" method="post" id="J_container" ></form>
                </div>
...
<div id="J_row_tpl" style="display: none;">
    <div class="layui-row" style="border: gray dashed 1px;padding: 10px;margin-top: 10px">
        ...
        <div class="layui-col-md4">
            <span>时间:</span>
            <input class="layui-input layui-input-inline J_start_time" name="start_time" type="text" style="width: 120px" />
            &nbsp;~&nbsp;<input class="layui-input layui-input-inline J_end_time" name="end_time" type="text" style="width: 120px" />
        </div>
        <div class="layui-col-md2">
            <span>链接:</span>
            <input class="layui-input layui-input-inline" name="link" type="text" style="width: 120px" />
        </div>
        <div class="layui-col-md2 row_textarea">
            <span>data1:</span>
            <textarea class="layui-input layui-input-inline" name="data1" type="text" style="width: 120px;" ></textarea>
        </div>
        <div class="layui-col-md2 ">
            <span>data2:</span>
        ...
        <div class="layui-col-md2">
            ...
            <button type="button" data-id="" data-locid="{{$loc_id}}" class="layui-btn J_delete layui-btn-danger">删除</button>
        </div>
    </div>
</div>
...
<script>
    ...
    layui.use(['upload', 'form','laydate'], function () {
        ...
        function addRecord(record) {
            ...
            tpl.find(".J_delete").attr('data-id',record.id);
            var adStatus = 0;
            var adStatusDesc = "禁用";
            if(record.status == 0) {
                adStatus=1;
        ...
        $("#J_add_ad").on('click',function () {
            $.get("{{route('backend.ad.item.insert')}}?loc_id={{$loc_id}}",function (res) {
                if(res.code == 1){
                    ...
                    let className = 'J_img_'+num;
                    tpl.find('.J_img').addClass(className);
                    ++num;
                    $("#J_container").prepend(tpl.html());
                    addrUploads('.'+className);
                    form.render();
                }
            })
        });
    ...
    $("#J_container").on('click','.J_delete',function () {
        var ele = $(this);
        layer.confirm('确定要执行删除吗?', {icon: 3, title:'提示',
            yes: function(index){
                me = ele;
                let id = me.data('id');
                let loc_id = me.data('locid');
                $.post("{{ route('backend.ad.item.delete') }}",{id:id,loc_id:loc_id},function (res) {
                    if (res.code === 1) {
                        layer.msg('操作成功', {
                            icon: 1,
                            shade: 0.3,
