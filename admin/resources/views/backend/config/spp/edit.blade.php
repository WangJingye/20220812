                    ...
                    <div class="layui-input-block">
                        @foreach ($ruleTypeMap as $ruleTypeCode=>$ruleTypeName)
                            @if($ruleTypeCode == $detail->rule_type)
                                <input type="radio" lay-filter="rule" name="rule_type" value="{{$ruleTypeCode}}"
                                       title="{{$ruleTypeName}}" checked>
                            @else
                                <input type="radio" lay-filter="rule" name="rule_type" value="{{$ruleTypeCode}}"
                                       title="{{$ruleTypeName}}">
                            @endif
                                ...
                                <div class="layui-input-block">
                                    @foreach ($prodBrandCollList as $prodBrandCollItem)
                                        @if($prodBrandCollItem->code == $detail->step_o)
                                            <input type="radio" lay-filter="brand" name="brand_coll"
                                                   value="{{$prodBrandCollItem->code}}"
                                                   title="{{$prodBrandCollItem->name}}" checked>
                                        @else
                                            <input type="radio" lay-filter="brand" name="brand_coll"
                                                   value="{{$prodBrandCollItem->code}}"
                                                   title="{{$prodBrandCollItem->name}}">
                                        @endif
                                    @endforeach
                                </div>
                            </div>
                                    ...
                                    @else
                                        <div class="layui-form-item" style="display:none;" id="child_block">
                                            ...
                                            <div class="layui-input-block">
                                                @if(1 == $detail->is_child)
                                                    <input type="radio" lay-filter="child" name="is_child" value="1"
                                                           title="是" checked>
                                                    <input type="radio" lay-filter="child" name="is_child" value="2"
                                                           title="否">
                                                @else
                                                    <input type="radio" lay-filter="child" name="is_child" value="1"
                                                           title="是">
                                        ...
                                        @foreach ($prodBrandCollList as $prodBrandCollItem)
                                            @if(isset($prodBrandCollItem->sub))
                                                @if($prodBrandCollItem->code == $detail->step_o)
                                                    <div class="layui-form-item sub-coll"
                                                         id="{{$prodBrandCollItem->code}}">
                                                        @else
                                                            <div class="layui-form-item sub-coll" style="display:none;"
                                                                 id="{{$prodBrandCollItem->code}}">
                                                                @endif
                                                                <div class="layui-input-block">
                                                                    @foreach ($prodBrandCollItem->sub as $subItem)
                                                                        @if(isset($detail->step_t) && !empty($detail->step_t) && $subItem->code == $detail->step_t)
                                                                            <input type="radio" lay-filter="sub_brand"
                                                                                   name="sub_brand_coll"
                                                                                   value="{{$subItem->code}}"
                                                                                   title="{{$subItem->name}}" checked>
                                                                        @else
                                                                            <input type="radio" lay-filter="sub_brand"
                                                                                   name="sub_brand_coll"
                                                                                   value="{{$subItem->code}}"
                                                                                    ...
                                                                                    <div class="layui-form-item"
                                                                                        ...
                                                                                        @else
                                                                                            <div class="layui-form-item"
                                                                                                ...
                                                                                                <div class="layui-input-block">
                                                                                                    <textarea
                                                                                                            placeholder="多个款号之间以逗号(,)间隔"
                                                                                                            name="style_number"
                                                                                                            class="layui-textarea">{{$detail->include_style_number}}</textarea>
                                                                                                </div>
                                                                                            </div>
                                                                                            <div class="layui-form-item">
                                                                                                <div class="layui-input-block">
...
<script>
    ...
    layui.use(['upload', 'form'], function () {
        ...
        form.on('submit(confirm)', function (data) {
            $.post("{{ route('backend.config.spp.edit') }}", data.field, function (res) {
                if (res.code === 1) {
                    layer.msg('操作成功', {
                        icon: 1,
                        shade: 0.3,
