    ...
    <div class="layui-col-md12">
        <div class="layui-card">
            <div class="layui-card-body">
                <form class="layui-form layui-form-pane" action="{{ route('dsb.prodstat.favoriteCount') }}" method="POST">
...
<script>
    ...
    table.render({
            ...
            , id: 'table_list'
            , height: 500
            , url: "{{ route('dsb.prodstat.favoriteCount') }}" //数据接口
            , page: true //开启分页
            , limits: [10]
            , limit: 10 //每页默认显示的数量
            , method:'post'
            , cols: [[ //表头
                ...
                , {field: 'scores', title: '商品收藏次数', width: 200, sort: false}
    ...
    $(".export").click(function(){
        var ins1 = table.render({
            elem: '#data_export',
            url: "{{ route('dsb.prodstat.favoriteExport') }}", //数据接口
            method: 'post',
            title: '商品收藏次数',
            where: {
    ...
    table.on('tool(list)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'edit'){
            layer.open({
                ...
                fixed: false,
                maxmin: true,
                content: "{{ route('dsb.prodstat.favoriteCount') }}?id="+data.id,
                end: function(){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('dsb.prodstat.favoriteCount') }}",
                data: {id: data.id},
