    ...
    <div class="layui-col-md12">
        <div class="layui-card">
            <div class="layui-card-body">
                <form class="layui-form layui-form-pane" action="{{ route('dsb.prodstat.prodTypeViewCount') }}" method="POST">
...
<script>
    ...
    table.render({
            ...
            , id: 'table_list'
            , height: 500
            , url: "{{ route('dsb.prodstat.prodTypeViewCount') }}" //数据接口
            , page: true //开启分页
            , limits: [10]
            , limit: 10 //每页默认显示的数量
            , method:'post'
            , cols: [[ //表头
                {field: 'prod_cat_id', title: '分类id', width: 300, sort: false}
                ,{field: 'prod_cat_name', title: '分类名称', width: 300, sort: false}
                ,{field: 'scores', title: '商品类别访问次数',    width: 300, sort: false}
            , {field: 'ref_date', title: '商品类别访问日期',    width: 300, sort: false}
    ...
    $(".export").click(function(){
        var ins1 = table.render({
            ...
            , cols: [[ //表头
                {field: 'prod_cat_id', title: '分类id'}
                ,{field: 'prod_cat_name', title: '分类名称'}
                ,{field: 'scores', title: '商品类别访问次数'}
                ,{field: 'ref_date', title: '商品类别访问日期'}
    ...
    table.on('tool(list)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'edit'){
            layer.open({
                ...
                fixed: false,
                maxmin: true,
                content: "{{ route('dsb.prodstat.prodTypeViewCount') }}?id="+data.id,
                end: function(){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('dsb.prodstat.prodTypeViewCount') }}",
                data: {id: data.id},
