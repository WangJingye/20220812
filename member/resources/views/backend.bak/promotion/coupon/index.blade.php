...
var dataTable = table.render({
    ...
    ,id: 'table_list'
    ,height: 500
    ,url: "{{ route('backend.promotion.coupon.dataList') }}" //数据接口
    ,page: true //开启分页
    ,method:'post'
    ,cols: [[ //表头
...
table.on('tool(list)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    if(layEvent === 'edit'){
        layer.open({
            ...
            maxmin: true,
            fixed: false,
            content: "{{ route('backend.promotion.coupon.edit') }}?id="+data.id,
            end: function(){
        ...
        layer.confirm('真的删除行么', function(index){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('backend.promotion.coupon.destroy') }}",
                data: {id:data.id},
...
$('#add').on('click', function () {
    layer.open({
        ...
        offset: 't',
        maxmin: true,
        content: "{{ route('backend.promotion.coupon.edit') }}",
        end: function(){
            var type = 'reload';
            active[type] ? active[type].call(this) : '';
        }
