...
{  
    ...
    public function __construct(){
        $this->fissionService = new FissionService();
    }
    ...
    function add(Request $request)
    {
        ...
        [
            ...
            'condition_value' => $info['condition_value']?implode(',',$info['condition_value']):[],// '条件值',
            'max_num' => $info['max_num']?? 1,// '赠送上线',
            'step' => 1,//$info['step']?? 1,// '是否多增   1赠1个      2多赠',
            'value_id' => $info['value_id']?implode(',',$info['value_id']):[],// '优惠券/优惠码id',
            'type' => 2,// '类型   1   优惠码  2   优惠卷',
        ];
        if(isset($info['id']))
        {
        ...
        {
            $count = \DB::table('fission')->where('name', $params['name'])->count();
            if($count)
            {
                $response['code'] = 0;
                $response['message'] = "已存在相同名称的裂变活动";
    ...
    public function registerAdd(Request $request)
    {
        $params = $request->all();
        //插入表
        $result = $this->fissionService->buildRelation($params);
        if($result['code'] == 1)
        {
        return response()->ajax('注册成功');
        }
        return  ['code' =>1 ,'msg' =>1];
    }
    ...
    {
        $result =  $this->fissionService::getFissionRank();
        $response['code'] = 1;
        $response['msg'] = '查询成功';
        $response['count'] = 10;
    ...
    /**
     * [getFissionInfo description]
     * @Author   Peien
     * @DateTime 2020-08-22T19:41:36+0800
     * @return   [type]                   [description]
     */
    public function getFissionInfo(Request $request)
    {
        $result = $this->fissionService->getFissionInfo($request->all());
        if($result['code'] == 1)
        {
