        ...
        <div class="layui-card-body">
            <form class="layui-form layui-form-pane" action="">
                ...
                <div class="layui-form-item">
                    ...
                    <a class="layui-btn sub layui-btn-primary" href="{{route('backend.promotion.cart.export')}}">导出</a>
                </div>
            </form>
...
@section('layui_script')
    ...
    table.on('tool(list)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'edit'){
            ...
            layer.confirm('真的删除行么', function(index){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('backend.promotion.cart.destroy') }}",
                    data: {id:data.id},
    ...
    $('#add').on('click', function () {
        layer.open({
            ...
            offset: 't',
            maxmin: true,
            content: "{{ route('backend.promotion.cart.edit') }}",
            end: function(){
                var type = 'reload';
                active[type] ? active[type].call(this) : '';
            }
